			    ...
			    script {
                    ...
                    withSonarQubeEnv('SonarServer') 
                    {
                       // sh "mvn sonar:sonar -Dsonar.projectKey=opendays "
                            sh "${scannerHome}/bin/sonar-scanner -X -Dsonar.projectKey=opendays -Dsonar.login=admin -Dsonar.password=password -Dsonar.scm.exclusions.disabled=true -Dsonar.java.binaries=./target"
                    }
                    sleep(10)
